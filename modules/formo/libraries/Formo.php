...
class Formo_Core {
	...
	public $_open = '<form action="{action}" method="{method}" class="{class}" name="{name}" id="{id}">';
	public $_close = '</form>';
	public $_action;
	public $_method = 'post';
	...
	public function __call($function, $values)
	{
		...
		// if all else fails, just use the last accessed object
	...
	public function validate($force_validation = FALSE)
	{
		...
		if ( ! $this->_sent AND ! $force_validation)
			return;
	...
	private function _all_filters($type)
	{
		...
		foreach ($this->find_elements() as $element)
		{
			...
			{
				$this->$element->run_filter($filter, $type);
			}
	...
	public function post_filters($function, $elements)
	{
		...
		foreach ($_functionas as $_function)
		{
			...
			{
				$_element = trim($_element);
				$this->post_filter(trim($_element), $_function);
			}
		}
	}
	...
	public static function include_file($type, $file)
	{
		...
		if ( ! $_file)
		{
			throw new Kohana_Exception($file.' plugin cannot be found.<br /><br />'.$path.'_core/'.$file.'.php<br />OR '.$path.'/'.$file.'.php');
		}
	...
	public function find_elements()
	{
		...
		{
			$hidden_fields = array_keys($this->_elements, 'hidden');
			$arr = array_keys($this->_elements);
			
			$arr = array_unique(array_merge($hidden_fields, $arr));
	...
	// generate opening tag for form, grouping hidden fields when wanted	
	public function open($include_hidden = FALSE, $only_open_tag = FALSE)
	{
		$search = array('/{action}/','/{method}/','/{class}/','/{name}/', '/{id}/');
		$replace = array($this->_action,$this->_method,$this->_class,$this->_formo_name, $this->_id);
		$str = preg_replace($search,$replace,$this->_open);
		
		if ($only_open_tag === TRUE)
			return $str;
			
		if ($include_hidden === TRUE)
		{
			foreach (array_keys($this->_elements, 'hidden') as $field)
			{
				$str.= $this->$field->get();
			}
